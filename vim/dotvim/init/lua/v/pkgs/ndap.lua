...
local function setup_mappings()
    ...
    m.group_begin('dap')
    m.nnore({ '<F4>', dap.terminate, desc = 'Terminate DAP debug' })
    m.nnore({ '<F5>', dap.continue, desc = 'Start/continue DAP debug' })
    m.nnore({ '<F6>', dap.restart, desc = 'Restart DAP debug' })
    m.nnore({ '<F7>', breakpoint_hit_condition, desc = 'Set hit condition breakpoint' })
...
return {
    ...
    dependencies = {
        ...
        {
            'Joakker/lua-json5',
            build = use.has_build and (IsWin() and 'powershell ./install.ps1' or './install.sh'),
        },
